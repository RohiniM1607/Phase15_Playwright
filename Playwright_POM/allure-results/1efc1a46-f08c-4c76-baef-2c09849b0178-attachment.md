...
Expected substring: "Welcome RohiniM"
Received string:    ""
```
      ...
      - generic [ref=e4]:
        ...
        - generic [ref=e9]:
          - generic [ref=e10]:
            ...
            - textbox [ref=e12]: RohiniM
          - generic [ref=e13]:
            - generic [ref=e14]: "Password:"
            - textbox [ref=e15]: Rohini_16
        - generic [ref=e17]:
...
```ts
  ...
  3  | 
  4  | test.describe('Login Test', ()=>{
  5  |     test.beforeEach(async({loginPage})=>{
  6  |         await loginPage.Navigate(process.env.BASE_URL!);
  7  |     })
  8  |     test('Valid Login', async({loginPage, dashboardPage})=>{
  9  |         await loginPage.clickLogin();
  10 |         await expect(loginPage.username).toBeVisible();
  11 |         await loginPage.enterCredentials(loginData.validUser.username,loginData.validUser.password);
  12 |         await loginPage.clickLoginButton();
> 13 |         await expect(await dashboardPage.verifyWelcomeUser()).toContain("Welcome RohiniM");
     |                                                               ^ Error: expect(received).toContain(expected) // indexOf
  14 |     })
  15 | 
  16 | })
