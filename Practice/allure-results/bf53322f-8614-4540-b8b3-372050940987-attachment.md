...
- Name: AutomationExercise\TC4_LogoutUser.test.ts >> Logout User
- Location: tests\AutomationExercise\TC4_LogoutUser.test.ts:3:1
...
Locator: getByText('Logged in as Sample')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('Logged in as Sample')
...
```ts
  ...
  10 |     await expect(page.getByText('Login to your account')).toBeVisible();
  11 |     await page.getByPlaceholder('Email Address').first().fill("sample.123@gmail.com");
  12 |     await page.getByPlaceholder('Password').fill("Sample@123");
  13 |     console.log("User Credentials Entered");
  14 |     await page.getByRole('button', {name: 'Login'}).click();
> 15 |     await expect(page.getByText('Logged in as Sample')).toBeVisible();
     |                                                         ^ Error: expect(locator).toBeVisible() failed
  16 |     console.log("User logged in successfully");
  17 |     await page.getByRole('link', {name:' Logout'}).click();
  18 |     await expect(page).toHaveURL('https://automationexercise.com/login');
  19 | })
  20 | 
```